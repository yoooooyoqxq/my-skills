...
function mapsSearch(query) {
  return `https://uri.amap.com/search?keyword=${encodeURIComponent(query)}&src=travel-page&callnative=1`;
}
...
function flightCard(journey, index) {
  ...
  return `
    <article class="flight-card" data-journey="${escapeHtml(journey.id)}">
      ...
      ${journey.travelers?.length ? `<div class="flight-card__travelers">乘机人：${escapeHtml(journey.travelers.join("、"))}</div>` : ""}
...
function navigationDestinations(item) {
  ...
  if (referencedPlaceIds.length) {
    return referencedPlaceIds.map((placeId) => state.data.places.find((place) => place.id === placeId)).filter(Boolean).map((place) => ({
      ...
      geo: Number.isFinite(Number(place.lng)) && Number.isFinite(Number(place.lat)) ? `${place.lng},${place.lat}` : ""
    }));
  }
...
function setupPlaceMap() {
  ...
  // 行程地点的地图按钮：点击直接唤起高德地图 App 导航（带经纬度），缺经纬度时回退到高德搜索
  const amapUrlFor = (link) => {
    const geo = String(link.dataset.mapGeo || "").trim().replace(/\s+/g, "");
    const label = encodeURIComponent(String(link.dataset.mapLabel || link.dataset.mapQuery || ""));
    if (/^-?\d+(\.\d+)?,-?\d+(\.\d+)?$/.test(geo)) {
      return `https://uri.amap.com/navigation?to=${geo},${label}&callnative=1`;
    }
    return safeExternalUrl(link.dataset.mapUrl) || mapsSearch(link.dataset.mapQuery);
  };
  document.addEventListener("click", (event) => {
    const link = event.target.closest("button[data-map-query]");
    if (!link) return;
    event.preventDefault();
    window.open(amapUrlFor(link), "_blank", "noopener,noreferrer");
  });
...
async function init() {
  try {
    // 优先从内嵌 <script type="application/json"> 读取（支持 file:// 直接打开）
    const inlineScript = document.getElementById("trip-data-inline");
    if (inlineScript && inlineScript.textContent.trim()) {
      state.data = JSON.parse(inlineScript.textContent);
    } else {
