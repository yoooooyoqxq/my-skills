...
// 生成高德地图链接：优先带经纬度直接唤起 App 导航，缺经纬度时回退到搜索
function gaodeNavUrlFor(place, query) {
  const keyword = encodeURIComponent(String(query || place?.lines?.[0] || place?.label || place?.name || place?.id || "").trim());
  const geo = place?.geo || place;
  const lng = Number(geo?.lng ?? geo?.lon);
  const lat = Number(geo?.lat);
  if (Number.isFinite(lng) && Number.isFinite(lat) && lng !== 0 && lat !== 0) {
    return `https://uri.amap.com/navigation?to=${lng},${lat},${keyword}&callnative=1`;
  }
  return `https://uri.amap.com/search?keyword=${keyword}&src=travel-page&callnative=1`;
}
...
function mapArtwork(source, selected, id, viewport) {
  ...
  svg.querySelector("title")?.replaceChildren(day.title);
  svg.querySelector("desc")?.replaceChildren("仅显示当天路线；地点圆点打开地图，交通图标查看行程。");
...
function setupRouteExplorer() {
  ...
  const showPopover = (pin, content, map = false) => {
    ...
    popover.setAttribute("role", "dialog"); popover.setAttribute("aria-label", map ? "地点高德地图" : "交通信息");
  ...
  document.addEventListener("click", (event) => {
    ...
    if (placePin) {
      ...
      const place = placeLayersFor(source).find((item) => item.id === placePin.dataset.placeId);
      showPopover(placePin, `<header><small>${escapeHtml(placePin.dataset.placeRole)}</small><strong data-popup-place-label>${escapeHtml(label)}</strong></header>
        ...
        <a class="popup-amap-nav" data-popup-external href="${gaodeNavUrlFor(place, query)}" target="_blank" rel="noopener noreferrer">用高德地图导航 ↗</a>
        <footer><small>点击上方按钮唤起高德地图 App，从当前位置导航到该地点。</small></footer>`, true);
      popover.dataset.mapRegion = source.id;
      popover.dataset.placeId = placePin.dataset.placeId;
      return;
    }
    ...
    if (option && popover) {
      ...
      const place = placeLayersFor(travelMapSource(state.data?.routeMap, popover.dataset.mapRegion)).find((item) => item.id === popover.dataset.placeId);
      $("[data-popup-external]", popover).href = gaodeNavUrlFor(place, query);
      return;
    }
