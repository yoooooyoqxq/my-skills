...
function gaodeMapUrl(place, city) {
  const keyword = encodeURIComponent(String(place?.lines?.[0] || place?.label || place?.name || place?.id || "").trim());
  ...
  return `https://uri.amap.com/search?keyword=${keyword}${cityParam}&src=travel-page&callnative=1`;
}
...
function travelOverviewArtwork(days, source = {}, options = {}) {
  ...
  <g id="overview-markers" inkscape:groupmode="layer" inkscape:label="地点标记">${placeLayers.map((place) => `<a href="${esc(gaodeMapUrl(place, place.city || "北京"))}" target="_blank" rel="noopener" style="cursor:pointer"><circle id="overview-point-${esc(place.id)}" cx="${place.x}" cy="${place.y}" r="10.5" fill="${esc(place.color)}" stroke="#fafaf4" stroke-width="2.5"/></a>`).join("")}</g>
  <g id="overview-place-names" inkscape:groupmode="layer" inkscape:label="地点名称" fill="#092653" stroke="#092653" stroke-width="0.4" stroke-linejoin="round" paint-order="stroke fill" font-family="'Times New Roman', 'Kaiti SC', STKaiti, KaiTi, 'Songti SC', serif" font-weight="700">${placeLayers.map((place) => `<a href="${esc(gaodeMapUrl(place, place.city || "北京"))}" target="_blank" rel="noopener" style="cursor:pointer"><text id="overview-label-${esc(place.id)}" x="${place.tx}" y="${place.ty}" text-anchor="${esc(place.anchor || "start")}" font-size="${place.size}">${multilineText(place.lines, place.tx)}</text></a>`).join("")}</g>
